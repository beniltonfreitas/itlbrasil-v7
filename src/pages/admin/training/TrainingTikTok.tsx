...
import { Music, Trash2, ExternalLink, Loader2 } from "lucide-react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { useTrainingVideos } from "@/hooks/useTrainingVideos";
import { tiktokVideoSchema, type TrainingVideoFormData } from "@/schemas/trainingVideoSchema";

export const TrainingTikTok = () => {
  const { videos, isLoading, addVideo, deleteVideo, isAdding } = useTrainingVideos('tiktok');
  
  const { register, handleSubmit, formState: { errors }, reset } = useForm<TrainingVideoFormData>({
    resolver: zodResolver(tiktokVideoSchema)
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-3xl font-bold">Treinamento - TikTok</h1>
      </div>
      
      <Card>
        <CardHeader>
          <CardTitle>Adicionar Vídeo do TikTok</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <Input 
                id="title"
                {...register('title')}
                placeholder="Ex: Tutorial rápido" 
                className="mt-1" 
              />
              {errors.title && (
