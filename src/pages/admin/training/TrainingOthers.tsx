...
import { Monitor, Trash2, ExternalLink, Loader2 } from "lucide-react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { useTrainingVideos } from "@/hooks/useTrainingVideos";
import { trainingVideoSchema, type TrainingVideoFormData } from "@/schemas/trainingVideoSchema";

export const TrainingOthers = () => {
  const { videos, isLoading, addVideo, deleteVideo, isAdding } = useTrainingVideos('outros');
  
  const { register, handleSubmit, formState: { errors }, reset } = useForm<TrainingVideoFormData>({
    resolver: zodResolver(trainingVideoSchema)
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-3xl font-bold">Treinamento - Outros Vídeos</h1>
      </div>
      
      <Card>
        <CardHeader>
          <CardTitle>Adicionar Vídeo de Outras Plataformas</CardTitle>
        </CardHeader>
        <CardContent>
