...
import { vimeoVideoSchema, type TrainingVideoFormData } from "@/schemas/trainingVideoSchema";

export const TrainingVimeo = () => {
  const { videos, isLoading, addVideo, deleteVideo, isAdding } = useTrainingVideos('vimeo');
  
  const { register, handleSubmit, formState: { errors }, reset } = useForm<TrainingVideoFormData>({
    resolver: zodResolver(vimeoVideoSchema)
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-3xl font-bold">Treinamento - Vimeo</h1>
      </div>
      
      <Card>
        <CardHeader>
          <CardTitle>Adicionar Vídeo do Vimeo</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <Input 
                id="title"
                {...register('title')}
                placeholder="Ex: Tutorial profissional" 
                className="mt-1" 
              />
              {errors.title && (
